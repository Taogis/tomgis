...
def prepare_dataForLaneNet(dataset_dir):
    train_input_names=[]
    train_output_names=[]
    val_input_names=[]
    val_output_names=[]
    ...
    cwd = os.getcwd()
    train_data_info_file = cwd + "/" + dataset_dir + "/" + "CULaneTrain.txt"
    with open(train_data_info_file, 'r') as file:
        for _info in file:
            info_tmp = _info.strip(' ').split()
            train_input_names.append("/home/mmtao/data/CULane" + info_tmp[0])
            train_output_names.append("/home/mmtao/data/CULane" + info_tmp[1])

    train_data_info_file = cwd + "/" + dataset_dir + "/" + "CULaneVal.txt"
    with open(train_data_info_file, 'r') as file:
        for _info in file:
            info_tmp = _info.strip(' ').split()
            val_input_names.append("/home/mmtao/data/CULane" + info_tmp[0])
            val_output_names.append("/home/mmtao/data/CULane" + info_tmp[1])

    test_data_info_file = cwd + "/" + dataset_dir + "/" + "test.txt"
    with open(test_data_info_file, 'r') as file:
        for _info in file:
            info_tmp = _info.strip(' ').split()
            test_input_names.append("/home/mmtao/data/CULane" + info_tmp[0])
            test_output_names.append("/home/mmtao/data/CULane" + info_tmp[1])
...
def load_outputimage(path):
    #image = cv2.cvtColor(cv2.imread(path,-1), cv2.COLOR_BGR2RGB)
    image = cv2.imread(path, cv2.IMREAD_COLOR)
    #ret, dst = cv2.threshold(image,0,255,cv2.THRESH_BINARY)
    label_img = image
